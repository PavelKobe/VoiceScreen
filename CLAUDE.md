...
**Подготовка к пилоту:** минимальный CRUD клиентов/вакансий через API, S3 для записей, разблокировать Telegram-бот, АОН-брендинг номера. Сквозной звонок и `/upload` уже работают.

## Прогресс

См. `ROADMAP.md` — чекбоксы обновляются по мере выполнения задач. Фаза 0 (скелет) и большая часть Фазы 1 (сквозной звонок + persist + scoring + recording) завершены. Из Фазы 2 готова `/upload` ручка с auth.
